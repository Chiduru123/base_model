...
def dataset_loader():
    train_image = read_image('/data/train-images-idx3-ubyte')
    train_label = read_label('/data/train-labels-idx1-ubyte')
    test_image = read_image('/data/t10k-images-idx3-ubyte')
...
class ConvNet(object):
    ...
    def evaluate(self, images, labels):
        ...
        for img, lab in zip(images, labels):
            predict_label, cache = self.feed_forward(img)
    ...
    def SGD(self, train_image, train_label, test_image, test_label, epochs, mini_batch_size, learning_rate):
        '''
        ...
        for j in range(epochs):
            ...
            for mini_batch_image, mini_batch_label in zip(mini_batches_image, mini_batches_label):
                ...
                self.update_mini_batch(mini_batch_image, mini_batch_label, learning_rate, mini_batch_size)     # 更新梯度
    ...
    def update_mini_batch(self, mini_batch_image, mini_batch_label, eta, mini_batch_size):
        ...
        nabla_b = [np.zeros(b.shape) for b in self.biases]  # nabla - 微分算子（哈密顿）
    ...
    def backprop(self, x, y):
        ...
        AL, caches = self.feed_forward(x)

        delta_fc3 = AL - y      # 交叉熵损失
        delta_fc2 = np.dot(self.weights[2].transpose(), delta_fc3) * relu_prime(caches[-2][0])  # full_connect2_z
        delta_fc1 = np.dot(self.weights[1].transpose(), delta_fc2) * relu_prime(caches[-3][0])    # full_connect1_z
        delta_straight_input = np.dot(self.weights[0].transpose(), delta_fc1)
        delta_pool2 = delta_straight_input.reshape(caches[1][1].shape)     # pool2

        delta_conv2 = pool_delta_error_bp(delta_pool2, caches[1][2]) * relu_prime(caches[1][0])

        delta_pool1 = conv(padding(delta_conv2, self.filters[1].shape[1] - 1), rot180(self.filters[1]).swapaxes(0, 3))

        delta_conv1 = pool_delta_error_bp(delta_pool1, caches[0][2]) * relu_prime(caches[0][0])

        # 求各参数的导数
        nabla_w2 = np.dot(delta_fc3, caches[-2][1].transpose())
        nabla_b2 = delta_fc3
        nabla_w1 = np.dot(delta_fc2, caches[-3][1].transpose())
        nabla_b1 = delta_fc2
        nabla_w0 = np.dot(delta_fc1, caches[-4][0].transpose())
        nabla_b0 = delta_fc1

        nabla_filters1 = conv_cal_w(delta_conv2, caches[0][1])      # pool1
...
def main():
    ...
    net = ConvNet()
    net.SGD(train_image, train_label, test_image, test_label, epochs=50, mini_batch_size=500, learning_rate=1e-5)
